...
func SelectOrCreateIdentity(executor shell.Executor, io cout.IO) (string, error) {
	identities := config.ListIdentities()

	if len(identities) == 0 {
		return createNewIdentity(executor, io)
	}

	options := []string{"Create new identity"}
	fingerprintMap := make(map[string]string)

	fingerprints := make([]string, 0, len(identities))
	for fp := range identities {
	...
	for _, fp := range fingerprints {
		...
		label := fmt.Sprintf("%s <%s> (%s...)", identity.Name, identity.Email, fp[:16])
		options = append(options, label)
		fingerprintMap[label] = fp
	}

	io.Infoln("Existing identities found:")
	selected, err := pterm.DefaultInteractiveSelect.WithOptions(options).Show()
	if err != nil {
		return "", fmt.Errorf("failed to get identity selection: %w", err)
	}

	if selected == "Create new identity" {
		return createNewIdentity(executor, io)
	}

	fingerprint := fingerprintMap[selected]
	io.Successfln("Using existing identity: %s", fingerprint)
	return fingerprint, nil
}

func createNewIdentity(executor shell.Executor, io cout.IO) (string, error) {
	...
	name, err := io.Input("Name", "")
	if err != nil {
		return "", err
	}

	email, err := io.Input("Email", "")
	if err != nil {
		return "", err
	}

	fingerprint, err := g.GenerateKeys(name, email)
	if err != nil {
	...
	if err := config.AddIdentity(fingerprint, identity); err != nil {
	...
	if err := initYubikeyForIdentity(g, fingerprint, name, email, io); err != nil {
		return "", err
	}
...
func SetupGPGForIdentity(fingerprint string, executor shell.Executor, io cout.IO) (*gpg.GPG, error) {
	configDir, err := config.Dir()
	if err != nil {
		return nil, err
	}
...
func initYubikeyForIdentity(g *gpg.GPG, fingerprint, userName, userEmail string, io cout.IO) error {
	...
	identity, _ := config.GetIdentity(fingerprint)
	if identity != nil {
		identity.YubikeyAdminPin = y.AdminPin
		identity.YubikeySerial = y.SerialNumber
		...
		if y.AdminPin != "manual" {
			...
			if errors.Is(err, gpg.ErrBadPIN) {
				...
				identity.YubikeyUserPin = "manual"
			} else if err == nil {
				slog.Debug("default user pin verified, saving to config")
				identity.YubikeyUserPin = "123456"
			}
		}

		if err := config.AddIdentity(fingerprint, identity); err != nil {
			slog.Warn("failed to update identity with yubikey info", "error", err)
		}
	}
